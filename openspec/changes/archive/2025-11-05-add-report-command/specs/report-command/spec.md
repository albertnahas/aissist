...
The report command generates professional, human-readable accomplishment reports from aissist data. Reports are formatted for brag documents, promotion cases, and manager communications.

## ADDED Requirements

### Requirement: Command Interface

The report command MUST be accessible as a Claude Code slash command that invokes the aissist CLI.
...
**Note:** This is a Claude Code slash command only (similar to `/log`). It is NOT registered as a separate CLI command in `src/index.ts`. The slash command invokes the aissist CLI tool directly via Bash.
...
- AI-powered summarization (defer to v2)
- PDF export (defer to phase 3)
- HTML export (defer to phase 3)
- Custom templates (defer to phase 3)
- Team/manager views (defer to phase 5)
- Email integration
- Calendar integration
- GitHub PR integration beyond existing history import

## Dependencies

- Existing utilities: `parseNaturalDate()`, `readMarkdown()`, `getStoragePath()`
...
### Unit Tests
- Timeframe parsing (7 scenarios)
- Data aggregation (7 scenarios)
- Report formatting (9 scenarios)
- Language transformation (6 scenarios)

### Integration Tests
- End-to-end report generation (all timeframes)
- File output operations
- Error handling flows

### Manual Tests
- Generate with real user data
- Verify professional language quality
- Test copy/paste to various platforms (Slack, email, docs)

## Success Metrics

1. Reports generate in < 5 seconds for 30-day timeframes
2. Generated markdown requires < 5 minutes of manual editing
3. Users report 80%+ of accomplishments are captured accurately
4. Zero formatting issues when sharing reports professionally
