...
## Purpose
Provide an optional `--smart` flag on the `aissist history log` CLI command that uses Claude Haiku to clean up rough input text and automatically link to the most relevant goal when a confident match exists.
